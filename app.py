    ...
    if st.button("Générer Export", type="primary"):
        if format_export == "CSV":
            ...
            st.download_button(
                ...
                file_name=f"rapport_{type_rapport.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.xlsx",
...
def recherche_avancee_page():
    ...
    with col2:
        ...
        st.download_button(
            "📥 Télécharger Excel",
            data=output.getvalue(),
            file_name=f"recherche_resultats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    ...
    with col3:
        ...
        st.download_button(
            "📥 Télécharger JSON",
            data=json_data,
            file_name=f"recherche_resultats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            mime="application/json",
...
def main():
    ...
    <div style='text-align: center; margin-top: 1rem; color: #6c757d; font-size: 0.9rem;'>
        🛡️ <strong>Insurance Sales Tracker</strong> - Application Complète (11 Modules) avec Sauvegarde Automatique
    </div>
    """, unsafe_allow_html=True)
