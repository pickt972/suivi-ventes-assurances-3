    ...
    if submitted:
        ...
        if nom_client and numero_reservation and types_assurance:
            ...
            if len(df_check) > 0 and numero_reservation in df_check['Numéro de réservation'].values:
                ...
                # POPUP SYMPA DE CONFIRMATION
                st.markdown(f"""
                <div class="success-popup">
                    <h2>🎉 VENTE ENREGISTRÉE AVEC SUCCÈS ! 🎉</h2>
                    <p><strong>Client :</strong> {nom_client}</p>
                    <p><strong>Réservation :</strong> {numero_reservation}</p>
                    <p><strong>Assurance(s) :</strong> {len(types_assurance)} type(s)</p>
                    <p><strong>💰 Commission totale :</strong> {commission_totale}€</p>
                    <p>✅ <em>Données sauvegardées automatiquement</em></p>
                </div>
                """, unsafe_allow_html=True)
                
                st.balloons()
                # time.sleep(1) retiré car problématique dans Streamlit
                st.rerun()
        else:
